...
async function importFile(filePath: string, filename: string) {
  ...
  const makePrefix = vehicleMake ? vehicleMake.toUpperCase().replace(/[^A-Z0-9]/g, '').substring(0, 6) + '-' : '';
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    let partNumber = extractText(cells[5]);
    
    if (!partName || !manufacturer || !category || !partNumber) continue;
    
    // Add make prefix to generic part numbers that don't already have a unique identifier
    // Check if part number looks like a manufacturer code (long alphanumeric) or generic
    const isGenericPartNumber = !partNumber.match(/^[0-9]{5,}/) && 
                                !partNumber.match(/^[A-Z]{2,3}[0-9]{4,}/) &&
                                !partNumber.match(/^[0-9A-Z]{8,}/);
    
    if (isGenericPartNumber && vehicleMake) {
      partNumber = makePrefix + partNumber;
    }
    ...
    try {
      ...
      console.error(`Error importing ${partNumber}:`, e);
    }
  }
...
async function main() {
  ...
  // Only process NEW makes that don't exist yet
  const newMakes = ['Audi', 'BMW', 'Fiat', 'Genesis', 'Hyundai', 'Jaguar', 'Land Rover', 
                    'Mercedes-Benz', 'Mercury', 'Mini', 'Scion', 'Smart', 'Tesla'];
  
  let files = fs.readdirSync(importDir)
    .filter(f => f.endsWith('.xls'))
    .filter(f => {
      const name = f.replace('.xls', '');
      return newMakes.some(m => m.toLowerCase() === name.toLowerCase());
    })
    .sort();
  ...
  console.log(`Importing ${files.length} new make files: ${files.join(', ')}`);
  let total = 0;
